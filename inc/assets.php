...
namespace Goodmotion\inc\assets;
...
function addScript()
{
  ...
  if (WP_ENV !== 'development') {
    ...
    $k = explode('.', $legacy);
    $token = $k[1];
    wp_enqueue_script('mercredi-theme-' . $token, $path . '/dist/' . $legacy, array(), $token, true);
    ...
    foreach ($files as $key => $value) {
      ...
      $k = explode('.', $file);
      $token = $k[1];
      wp_enqueue_script('mercredi-theme-' . $token, $path . '/dist/' . $file, array(), $token, true);
    }
  } else {
    // development
    wp_enqueue_script('mercredi-theme', 'http://localhost:3000/main.js', []);
  }
}
...
function enqueue_scripts()
{
  add_filter('script_loader_tag', function ($tag, $handle, $src) {
    if (strpos($handle, 'mercredi-theme') === false) {
      return $tag;
    }
